...
export type Action =
  | { type: 'SET_AUTH'; payload: { user: Omit<User, 'socketId'>; instanceId: string; channelId: string } }
  ...
  | { type: 'ROLL_STARTED'; payload: { rollId: string; userId: string; username: string; dice: string } }
  | { type: 'ROLL_REVEALED'; payload: { entry: RollEntry; watcherIds: string[] } }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'USER_JOINED':
      return {
        ...state,
        users: [...state.users.filter(u => u.userId !== action.payload.user.userId), action.payload.user],
      };
    case 'USER_LEFT':
      return { ...state, users: state.users.filter(u => u.userId !== action.payload.userId) };
    case 'ROLL_STARTED': {
      const { rollId, userId, username, dice } = action.payload;
      return { ...state, pendingRolls: { ...state.pendingRolls, [rollId]: { rollId, userId, username, dice } } };
    }
    case 'ROLL_REVEALED': {
    ...
    case 'ROLL_CANCELLED': {
      return { ...state, pendingRolls: omit(state.pendingRolls, action.payload.rollId) };
    }
    default:
      return state;
  }
