...
export function RollToast() {
  ...
  const visible = state.rollHistory.filter((e: RollEntry) => visibleIds.has(e.id));
                ...
                {visible.map(entry => {
                  ...
                  return (
                    <motion.div
                      ...
                      layout
                    >
                      <div className={`roll-toast__row${crit ? ' roll-toast__row--crit' : ''}`}>
                        <span className="roll-toast__name">{entry.username}</span>
                        <span className="roll-toast__dice">{entry.dice}</span>
                        <div className="roll-toast__results">
                          {entry.results.length > 1 &&
                            entry.results.map((r, i) => <Pip key={i} value={r} />)}
                          {entry.results.length > 1 &&
                            <span className="roll-toast__sep">→</span>}
                          <span className="roll-toast__total">{entry.total}</span>
                        </div>
                      </div>
                    </motion.div>
                  );
